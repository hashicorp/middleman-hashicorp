...
class Middleman::HashiCorpExtension
  describe "#github_url" do
    before(:all) do
      ...
      app = middleman_app
      Middleman::HashiCorpExtension.new(app,
      ...
      @instance = app.generic_template_context # unclear why this needs to be set after, but it must
    end

    it "returns the project's GitHub URL if no argument is supplied" do
      expect(@instance.github_url).to match("https://github.com/hashicorp/this_project")
    end

    it "returns false if github_slug has not been set" do
      slugless_app = Middleman::Application.new()
      ...
      slugless_instance = slugless_app.generic_template_context
      expect(slugless_instance.github_url).to eq(false)
    end
  end
  ...
  describe "#path_in_repository" do
    before(:each) do
      ...
      @instance = app.generic_template_context
    end
    ...
    context "when a resource's path string is not within its source_file string" do
      it "returns a resource's path relative to its source repository's root directory" do
        ...
        path_in_repository = @instance.path_in_repository(current_page)
  ...
  describe "#pretty_arch" do
    ...
    [
      ...
      it "converts #{i} to #{e}" do
        expect(@instance.pretty_arch(i)).to eq(e)
      end
    end
  end
  ...
  describe "#pretty_os" do
    ...
    [
      ...
      it "converts #{given} to #{expected}" do
        expect(@instance.pretty_os(given)).to eq(expected)
      end
    end

    it "should capitalize unknown OS" do
      expect(@instance.pretty_os("hashicorplinux")).to eq("Hashicorplinux")
    end
  end
end
